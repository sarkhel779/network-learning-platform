...
describe("DHCP progress migration", () => {
  it("mirrors the runtime manifest in one transactional idempotent migration", () => {
    const sql = readFileSync(resolve("supabase/migrations/202609110001_add_dhcp_progress.sql"), "utf8");
    ...
    expect(sql).toContain("'lesson_dhcp_and_automatic_address_configuration', 1, 19");
    ...
    const rows = [...sql.matchAll(/^  \('path_networking_foundations', 'lesson_dhcp_and_automatic_address_configuration', 1, '([^']+)', (\d+), '([^']+)', '([^']+)', '([^']+)', true\)[,;]?$/gm)];
