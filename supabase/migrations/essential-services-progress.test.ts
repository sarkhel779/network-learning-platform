...
describe("essential services progress migration", () => {
  it("mirrors the runtime manifest in one transactional idempotent migration", () => {
    const sql = readFileSync(resolve("supabase/migrations/202609110003_add_essential_services_progress.sql"), "utf8");
    ...
    expect(sql).toContain("'lesson_http_https_tls_and_essential_network_services', 1, 24");
    ...
    const rows = [...sql.matchAll(/^  \('path_networking_foundations', 'lesson_http_https_tls_and_essential_network_services', 1, '([^']+)', (\d+), '([^']+)', '([^']+)', '([^']+)', true\)[,;]?$/gm)];
