...
alter table public.page_views
  add column account_id uuid references auth.users(id) on delete set null;

create index page_views_account_created_idx on public.page_views(account_id, created_at);

revoke all on function public.record_page_view(uuid, text, text, uuid) from public, anon, authenticated;
drop function public.record_page_view(uuid, text, text, uuid);
...
begin
  ...
  insert into public.page_views(event_id, path, visitor_id, account_id)
  values (p_event_id, p_path, p_visitor_id, auth.uid())
...
create or replace function public.admin_unique_visitor_count(p_from timestamptz, p_to timestamptz)
returns bigint
...
begin
  ...
  return (
    select count(distinct coalesce(account_id::text, visitor_id::text)) from public.page_views
    where created_at >= p_from and created_at < p_to
      and (account_id is not null or visitor_id is not null)
  );
end;
$$;
