...
describe("DNS progress migration", () => {
  it("mirrors the runtime manifest in one transactional idempotent migration", () => {
    const sql = readFileSync(resolve("supabase/migrations/202609110002_add_dns_progress.sql"), "utf8");
    ...
    expect(sql).toContain("'lesson_dns_and_name_resolution', 1, 21");
    ...
    const rows = [...sql.matchAll(/^  \('path_networking_foundations', 'lesson_dns_and_name_resolution', 1, '([^']+)', (\d+), '([^']+)', '([^']+)', '([^']+)', true\)[,;]?$/gm)];
