...
const { getViewer, recordLearnerProgress } = vi.hoisted(() => ({
  getViewer: vi.fn(),
...
beforeEach(() => { vi.clearAllMocks(); getViewer.mockResolvedValue({ id: "user-1" }); recordLearnerProgress.mockResolvedValue({ ok: true, progress: {} }); });

describe("POST /api/learning/progress", () => {
  it("rejects anonymous requests", async () => { getViewer.mockResolvedValue(null); expect((await POST(request(body))).status).toBe(401); });
