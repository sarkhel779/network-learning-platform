...
import { TopicIcon } from "@/features/home/topic-icon";
...
const topics = [
  { label: "TCP/IP Model", slug: "osi-and-tcp-ip-models#tcp-ip-model", icon: "tcp-ip" },
  { label: "OSI Model", slug: "osi-and-tcp-ip-models#osi-model", icon: "osi" },
  ...
  return (
    <main id="main-content" className="home-refresh">
      ...
      <section className="home-section" aria-labelledby="core-heading">
        ...
        <div className="home-topics">{topics.map((topic) => <Link key={topic.label} href={topic.slug === "tcp-udp-and-ports" ? `/paths/${pathway.slug}` : `/learn/${pathway.slug}/${topic.slug}`}><span className="home-topic-icon" aria-hidden="true"><TopicIcon kind={topic.icon} /></span><strong>{topic.label}</strong></Link>)}</div>
      </section>
