...
import { listLearners, type WaitlistStatusFilter } from "@/features/admin/admin.repository";
import type { LearnerRow } from "@/features/admin/admin.types";

type Props = { searchParams: Promise<{ q?: string; page?: string; waitlist?: string; from?: string; to?: string }> };

const waitlistFilters: WaitlistStatusFilter[] = ["joined", "unsubscribed", "none"];
const waitlistFilterLabels: Record<WaitlistStatusFilter, string> = { joined: "Joined", unsubscribed: "Unsubscribed", none: "Never joined" };
...
export default async function UsersPage({ searchParams }: Props) {
  ...
  const waitlistStatus = waitlistFilters.find((status) => status === params.waitlist);
  const fromDate = parseDateParam(params.from);
  const toDate = parseDateParam(params.to);
  const joinedFrom = fromDate ? `${fromDate}T00:00:00.000Z` : undefined;
  const joinedTo = toDate ? new Date(new Date(`${toDate}T00:00:00.000Z`).getTime() + 86400000).toISOString() : undefined;
  let result: { rows: LearnerRow[]; total: number } | null = null;
  try {
    result = await listLearners({ query, offset: (page - 1) * 20, limit: 20, waitlistStatus, joinedFrom, joinedTo });
  } catch {
  ...
  return <main className="admin-page" id="main-content">
    ...
    <form action="/admin/users" method="get" className="admin-search">
      ...
      <div className="admin-search__filters">
        <label htmlFor="admin-user-waitlist">Waitlist</label>
        <select id="admin-user-waitlist" name="waitlist" defaultValue={waitlistStatus ?? ""}>
          <option value="">All</option>
          {waitlistFilters.map((status) => <option key={status} value={status}>{waitlistFilterLabels[status]}</option>)}
        </select>
        <label htmlFor="admin-user-from">Joined from</label>
        <input id="admin-user-from" name="from" type="date" defaultValue={fromDate ?? ""} />
        <label htmlFor="admin-user-to">Joined to</label>
        <input id="admin-user-to" name="to" type="date" defaultValue={toDate ?? ""} />
      </div>
    </form>
    {!result ? <p role="status">The learner directory is temporarily unavailable. Please try again.</p> : result.rows.length === 0 ? <p>No learners found.</p> : <div className="admin-table-scroll"><table className="admin-table"><thead><tr><th scope="col">Learner</th><th scope="col">Joined</th><th scope="col">Action</th></tr></thead><tbody>{result.rows.map((user) => <tr key={user.id}><td><strong>{user.displayName || "Unnamed learner"}</strong><small>{user.email}</small></td><td>{new Date(user.createdAt).toLocaleDateString("en-IN")}</td><td><Link href={`/admin/users/${user.id}`}>View profile</Link></td></tr>)}</tbody></table></div>}
