...
<SectionContinue itemId="vlans_access_ports_and_trunks_section_why_vlans_exist" anchor="why-vlans-exist" />
...
<SectionContinue itemId="vlans_access_ports_and_trunks_section_access_ports_and_membership" anchor="access-ports-and-membership" />
...
<VlanMembershipPlayer progressItemId="vlans_access_ports_and_trunks_interactive_interactive_vlan_membership" />
...
<h3>Tagged frame format</h3>

<ProtocolFormatDiagram kind="vlan" />
...
<SectionContinue itemId="vlans_access_ports_and_trunks_section_trunks_and_802_1q" anchor="trunks-and-802-1q" />
...
<VlanTagJourneyPlayer progressItemId="vlans_access_ports_and_trunks_interactive_interactive_tag_journey" />
...
In this example, both gateway subinterfaces use the router's physical `Fa0/0` port. The switch-facing `Fa1/0/1` port is an 802.1Q trunk that allows VLANs 10 and 20. The red host sends to gateway `10.10.10.254` on `Fa0/0.10`; the router forwards toward `20.20.20.1` through `Fa0/0.20`. One physical trunk carries the tagged frame in both directions, with a different VLAN ID on the return trip.

<RouterOnStickPlayer />

Without JavaScript, trace the path as red host → VLAN 10 access port → VLAN 10 tagged trunk → router `Fa0/0.10` → Layer 3 routing → router `Fa0/0.20` → VLAN 20 tagged frame down the same trunk → VLAN 20 access port → green host. The router creates a new Ethernet frame for the destination VLAN.
...
<SectionContinue itemId="vlans_access_ports_and_trunks_section_allowed_vlans_and_routing_boundary" anchor="allowed-vlans-and-routing-boundary" />
