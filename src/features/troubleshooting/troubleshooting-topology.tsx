...
export function TroubleshootingTopology({ topology, activePath = [] }: Props) {
  ...
  return <section className="troubleshooting-topology" aria-labelledby={titleId}>
    ...
    <div className="troubleshooting-topology__map" role="img" aria-label="Branch troubleshooting topology">
      {topology.nodes.map((node, index) => <div className="troubleshooting-topology__hop" key={node.id}>
        ...
        {topology.links[index] ? <div className="troubleshooting-topology__link" aria-hidden="true">
          <span>{topology.links[index].fromInterface}</span><i /><span>{topology.links[index].toInterface}</span>
        </div> : null}
      </div>)}
    </div>
  </section>;
