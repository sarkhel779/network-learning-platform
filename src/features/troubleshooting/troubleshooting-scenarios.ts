...
export const guidedBranchPortalIncident = parse({
  ...
  tests: [
    ...
    { id: "inspect-vlan", label: "Inspect client switchport", command: "show interfaces Gi1/0/18 switchport", risk: "read-only", timeCost: 2, expectedFaultId: "wrong-access-vlan", evidence: { kind: "cli", title: "Switchport state", body: "Administrative access VLAN: 30; expected VLAN: 20." }, restoredEvidence: { kind: "cli", title: "Restored switchport state", body: "Administrative access VLAN: 20; operational access VLAN: 20." } },
    { id: "inspect-route", label: "Look up the portal route", command: "show ip route 203.0.113.80", risk: "read-only", timeCost: 2, expectedFaultId: "wrong-specific-route", evidence: { kind: "table", title: "Route selection", body: "203.0.113.80/32 via 10.20.99.1 overrides the valid /24 path." }, restoredEvidence: { kind: "table", title: "Restored route selection", body: "203.0.113.0/24 uses the approved edge-firewall next hop; no stale /32 remains." } },
    { id: "inspect-dns", label: "Resolve the portal name", command: "nslookup portal.example.test", risk: "read-only", timeCost: 1, expectedFaultId: "stale-portal-dns", evidence: { kind: "observation", title: "Resolver answer", body: "portal.example.test returns retired address 203.0.113.70 instead of 203.0.113.80." }, restoredEvidence: { kind: "observation", title: "Restored resolver answer", body: "portal.example.test returns the active address 203.0.113.80." } },
...
export const proBranchPortalIncident = parse({
  ...
  tests: [
    ...
    { id: "compare-paths", label: "Compare forward and return routing", command: "show route 203.0.113.80; show route 10.20.20.42", risk: "read-only", timeCost: 3, expectedFaultId: "asymmetric-stateful-return", evidence: { kind: "table", title: "Path comparison", body: "Forward traffic crosses edge-firewall; return traffic selects the branch-router bypass." }, restoredEvidence: { kind: "table", title: "Restored path comparison", body: "Forward and return routes both cross the edge firewall holding session state." } },
    ...
    { id: "compare-dns", label: "Compare cached and authoritative DNS", command: "ipconfig /displaydns; nslookup portal.example.test 10.20.50.53", risk: "read-only", timeCost: 2, expectedFaultId: "stale-dns-cache", evidence: { kind: "cli", title: "DNS comparison", body: "Client cache returns 203.0.113.70; resolver returns 203.0.113.80." }, restoredEvidence: { kind: "cli", title: "Restored DNS comparison", body: "Client cache and resolver both return 203.0.113.80." } },
