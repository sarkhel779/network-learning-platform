...
export function LessonProgressProvider({ viewerId, manifest, initialProgress, children }: {
  ...
  const send = useCallback(async (event: PendingProgressEvent) => {
    if (activeKeys.current.has(event.idempotencyKey)) return false;
    activeKeys.current.add(event.idempotencyKey);
    ...
    try {
      ...
      activeKeys.current.delete(event.idempotencyKey);
    }
  }, [store]);

  const flush = useCallback(async (onlyItemId?: string) => {
    for (const event of store.list()) {
      const item = manifest.items.find((candidate) => candidate.itemId === event.itemId);
      if (event.itemKind !== "knowledge_check" || item?.kind !== "knowledge_check") {
        store.remove(event.idempotencyKey);
        continue;
      }
  ...
  useEffect(() => {
    ...
    const retryTimer = window.setInterval(() => { void flush(); }, 5_000);
  ...
  const complete = useCallback(async (itemId: string, options?: CompletionOptions) => {
    ...
    if (item.kind !== "knowledge_check") return true;
    const answerCorrect = options?.answerCorrect ?? false;
    if (store.list().some((event) => event.itemId === itemId && event.itemKind === "knowledge_check" && event.answerCorrect === answerCorrect)) return false;
    ...
    const event: PendingProgressEvent = { ...common, eventType: "knowledge_check_attempted", itemKind: "knowledge_check", answerCorrect };
