  ...
  return parsePacketFlowScenario({ id: journey.id, title: journey.title, description: journey.description, defaultSpeed: 1, devices: journey.devices, links: journey.links,
    steps: journey.steps.map((step) => ({ id: step.id, title: step.title, explanation: step.explanation, durationMs: 2400, activeDeviceIds: step.activeDeviceIds, activeLinkIds: step.activeLinkIds,
      ...
      detailFields: [
        ...
        { label: "Selected next hop", value: step.selectedNextHop ?? "No next hop selected", layer: "context" },
        { label: "Selected outgoing interface", value: step.selectedOutgoingInterface ?? "No outgoing interface selected", layer: "context" },
...
export function HopByHopForwardingPlayer({ progressItemId }: { progressItemId?: string }) {
  ...
  const [atFinalStep, setAtFinalStep] = useState(false);
  ...
  return <section aria-labelledby="hop-by-hop-title" className="hop-by-hop-player">
    ...
    <fieldset><legend>Choose a forwarding journey</legend>{forwardingJourneys.map((candidate, candidateIndex) => <label key={candidate.id}><input checked={index === candidateIndex} name="forwarding-journey" onChange={() => { setIndex(candidateIndex); setAtFinalStep(false); }} type="radio" />{candidate.title}</label>)}</fieldset>
    <PacketFlowPlayer autoplay inspectionDepthControl key={journey.id} onStepChange={(_, final) => setAtFinalStep(final)} progressItemId={progressItemId} scenario={scenario} suppressHeading />
    {atFinalStep ? <aside aria-label="Journey outcome" className={`forwarding-outcome forwarding-outcome--${journey.terminal.kind}`} role="status">
