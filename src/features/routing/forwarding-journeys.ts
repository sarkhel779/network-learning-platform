...
export type ForwardingStep = Readonly<{
  ...
  selectedNextHop?: string;
  selectedOutgoingInterface?: string;
...
function successfulJourney(family: AddressFamily): ForwardingJourney {
  ...
  return {
    ...
    steps: [
      ...
      { ...base, id: "lookup-r1", kind: "lookup", title: "2. Router 1 selects a route", explanation: "The longest matching prefix selects the link toward Router 2.", selectedRouteId: "r1-specific", selectedNextHop: ipv4 ? "192.0.2.2" : "2001:db8:12::2", selectedOutgoingInterface: "R1 Gi0/1", ingressDevice: "r1", egressDevice: "r2", activeDeviceIds: ["r1"], activeLinkIds: [], sourceMac: "00:11:22:33:44:10", destinationMac: "00:11:22:33:44:01", hopLimit: 64 },
      { ...base, id: "forward-r1", kind: "forward", title: "3. Router 1 forwards", explanation: `Router 1 decrements ${ipv4 ? "TTL" : "Hop Limit"} and creates a new Layer 2 header for the next link.`, selectedRouteId: "r1-specific", selectedNextHop: ipv4 ? "192.0.2.2" : "2001:db8:12::2", selectedOutgoingInterface: "R1 Gi0/1", ingressDevice: "r1", egressDevice: "r2", activeDeviceIds: ["r1", "r2"], activeLinkIds: ["r1-r2"], sourceMac: "00:aa:00:00:01:02", destinationMac: "00:bb:00:00:02:01", hopLimit: 63 },
      { ...base, id: "lookup-r2", kind: "lookup", title: "4. Router 2 performs a fresh lookup", explanation: "Router 2 independently selects its directly connected destination network.", selectedRouteId: "r2-connected", selectedNextHop: "Directly connected destination", selectedOutgoingInterface: "R2 Gi0/1", ingressDevice: "r2", egressDevice: "destination", activeDeviceIds: ["r2"], activeLinkIds: [], sourceMac: "00:aa:00:00:01:02", destinationMac: "00:bb:00:00:02:01", hopLimit: 63 },
      { ...base, id: "forward-r2", kind: "forward", title: "5. Router 2 forwards", explanation: `Router 2 decrements ${ipv4 ? "TTL" : "Hop Limit"} once and rewrites Layer 2 for the final link.`, selectedRouteId: "r2-connected", selectedNextHop: "Directly connected destination", selectedOutgoingInterface: "R2 Gi0/1", ingressDevice: "r2", egressDevice: "destination", activeDeviceIds: ["r2", "destination"], activeLinkIds: ["r2-destination"], sourceMac: "00:cc:00:00:02:02", destinationMac: "00:dd:00:00:03:20", hopLimit: 62 },
...
function exceptionalJourney(id: "no-route" | "hop-limit-expired" | "unresolved-next-hop"): ForwardingJourney {
  ...
  return { id, title, description: explanation, family: "ipv6", source: "2001:db8:10::10", destination, initialHopLimit: id === "hop-limit-expired" ? 1 : 64, devices, links, terminal: { kind, reason, deviceId: "r1", ...(kind === "discarded" ? { icmpNote: "An ICMP error may be generated when policy and reachability permit." } : {}) }, steps: [
    ...
    { id: "terminal", kind, title: `2. ${title}`, explanation, selectedRouteId: id === "unresolved-next-hop" ? "r1-static" : undefined, selectedNextHop: id === "unresolved-next-hop" ? "2001:db8:12::2" : undefined, selectedOutgoingInterface: id === "unresolved-next-hop" ? "R1 Gi0/1" : undefined, ingressDevice: "r1", egressDevice: "r1", activeDeviceIds: ["r1"], activeLinkIds: [], sourceIp: "2001:db8:10::10", destinationIp: destination, sourceMac: "Not forwarded", destinationMac: "Not forwarded", hopLimit: id === "hop-limit-expired" ? 0 : 64 },
  ] };
}
