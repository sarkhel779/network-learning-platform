...
export function HeaderSearch({ lessons }: { lessons: HeaderSearchLesson[] }) {
  ...
  return <div className="header-search" role="search" onBlur={(event) => {
    ...
    <input type="search" aria-label="Search lessons" placeholder="Search topics, e.g. subnetting..."
      value={query} onChange={(event) => { setQuery(event.target.value); setOpen(true); }}
      onFocus={() => setOpen(true)} onKeyDown={(event) => { if (event.key === "Escape") setOpen(false); }} />
    {open && normalizedQuery && <div className="header-search__results" aria-label="Lesson search results">
      {matches.length ? matches.map((lesson) => <Link key={lesson.href} href={lesson.href} onClick={() => { setQuery(""); setOpen(false); }}><strong>{lesson.title}</strong><small>{lesson.objective}</small></Link>) : <p>No matching lessons yet.</p>}
    </div>}
  </div>;
}
