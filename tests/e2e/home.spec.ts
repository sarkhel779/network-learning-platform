...
test("introduces the beginner networking pathway", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("heading", { level: 1, name: /understand networks.*one packet at a time/i })).toBeVisible();
  await expect(page.getByRole("link", { name: /explore networking foundations/i })).toHaveAttribute(
    "href",
